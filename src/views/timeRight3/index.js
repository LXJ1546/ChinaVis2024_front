...
const TimeRight3 = () => {
  ...
  const handleClick = () => {
    console.log('点击切换')
    setSpecialPicMode(!specialPicMode)
  }
  useEffect(() => {
    drawRadar()
    drawTimeTitle()
  ...
  const drawSpecialPic1 = () => {
    ...
    const dynamicData = [random(), random(), random()]
    myChart.setOption({
      series: [
        {
  ...
  const drawSpecialPicMode2 = () => {
    drawSpecialPic1Mode2()
    drawSpecialPic2Mode2()
    drawSpecialPic3Mode2()
  }

  const drawSpecialPic1Mode2 = () => {
    const specialInstance = echarts.getInstanceByDom(specialRef.current)
    if (specialInstance) {
      specialInstance.dispose()
    }
